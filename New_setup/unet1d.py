...
class UNet1D(nn.Module):
    ...
    def forward(self, x):  # x:[B,C,T]
        ...
        logits = torch.clamp(logits, -8.0, 8.0).squeeze(1)  # [B,T]
...
def load_edf_windows(edf_path, json_path, cfg):
    ...
    raw.pick_channels(cfg["data"]["channels"])
...
def split_data(X, y, ratios=(0.7, 0.15, 0.15), seed=42):
    ...
    n1 = int(ratios[0] * len(X)); n2 = int((ratios[0] + ratios[1]) * len(X))
...
def create_loaders(X, y, batch, workers, sampler_type="normal", seed=42):
    ...
    train_ds = EEGDataset(Xtr, ytr); val_ds = EEGDataset(Xva, yva); test_ds = EEGDataset(Xte, yte)
    ...
    if sampler_type == "undersample":
        ...
        labels = (ytr.sum(1) > 0).astype(int)
        class_count = np.bincount(labels); weight = 1.0 / class_count
...
def roc_curve_manual(y_true: np.ndarray, y_scores: np.ndarray, n_thresholds: int = 101):
    ...
    fpr = fpr[order]; tpr = tpr[order]; thresholds = thresholds[order]
...
# ------------------------
def main():
    cfg = CONFIG  # import from config.py
    ...
    def autocast_ctx():
        try:
            return torch.autocast(device_type=device.type, dtype=torch.float16, enabled=(device.type=="cuda") and bool(cfg["trainer"]["amp"]))
        except TypeError:
    ...
    if x_npy and y_npy and Path(x_npy).exists() and Path(y_npy).exists():
        X = np.load(x_npy, mmap_mode="r"); y = np.load(y_npy, mmap_mode="r")
    else:
    ...
    if use_wandb:
        run_name = wb.get("name") or f"{cfg['logging'].get('run_name','unet1d')}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    #  TensorBoard
    """writer = None
    if bool(cfg["logging"].get("use_tensorboard", False)) and _HAS_TB:
        run = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        log_dir = Path(cfg["logging"].get("tensorboard_dir", "./tb_logs")) / run
    ...
    def run_epoch(loader, train=True):
        ...
        for xb, yb in loader:
            ...
            with autocast_ctx():
                logits = model(xb)               # [B,T]
                loss = criterion(logits, yb)     # logits + targets in [0,1]
            if train:
    ...
    for epoch in range(1, int(cfg["trainer"]["epochs"]) + 1):
        ...
        with torch.no_grad():
            for xb, yb in val_loader:
                ...
                probs.append(p); trues.append(yb.detach().cpu().numpy())
...
"""
        if writer:
            for k, v in log_payload.items():
                if k in ("epoch", "val/TP", "val/TN", "val/FP", "val/FN"):  # TB wants scalars; ints are ok
        ...
        if use_wandb:
            ...
            wandb.log({**log_payload, "val/roc_curve": wandb.Image(fig)}, step=epoch)
            plt.close(fig)
        else:
            print({k: (int(v) if isinstance(v, bool) or isinstance(v, int) else float(v)) for k, v in log_payload.items()})

        print(f"Epoch {epoch}: loss={tr_loss:.4f} | thr={thr:.2f} | "
              f"F1={m['f1']:.4f} P={m['prec']:.3f} R={m['rec']:.3f} Acc={m['acc']:.3f} "
              f"| TP={m['TP']} FP={m['FP']} TN={m['TN']} FN={m['FN']} | AUC={roc_auc:.3f}")
