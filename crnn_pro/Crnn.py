...
def _extract_core_id(fname: str) -> str:
    """
    Extract core like P002_1 from either:
      P002_1_raw.edf
      sleep_block_spindle_output_P002_1.json
    """
    base = os.path.splitext(os.path.basename(fname))[0]
    m = re.search(r"(P\d{3}_\d+)", base, flags=re.IGNORECASE)
    return m.group(1) if m else base

def _pair_edf_json(edf_dir: str, json_dir: str):
    edfs = sorted([f for f in os.listdir(edf_dir) if f.lower().endswith(".edf")])
    jsons = sorted([f for f in os.listdir(json_dir) if f.lower().endswith(".json")])

    if not edfs:
        raise RuntimeError(f"No EDFs in {edf_dir}")
    if not jsons:
        raise RuntimeError(f"No JSONs in {json_dir}")

    # Build JSON index by core id
    json_index = {}
    for j in jsons:
        core = _extract_core_id(j)
        json_index.setdefault(core, []).append(j)

    pairs = []
    for edf in edfs:
        core = _extract_core_id(edf)
        cands = json_index.get(core, [])

        if len(cands) == 1:
            pairs.append((os.path.join(edf_dir, edf), os.path.join(json_dir, cands[0])))
        elif len(cands) == 0:
            raise RuntimeError(
                f"No JSON found for EDF={edf} (core={core}). "
                f"Example JSON files: {jsons[:5]}"
            )
        else:
            # if duplicates exist, prefer the exact sleep_block_spindle_output_* one
            pref = [c for c in cands if c.lower().startswith("sleep_block_spindle_output_")]
            if len(pref) == 1:
                pairs.append((os.path.join(edf_dir, edf), os.path.join(json_dir, pref[0])))
            else:
                raise RuntimeError(
                    f"Multiple JSON matches for core={core}: {cands}. "
                    f"Please keep only one label file per recording."
                )

    return pairs

def _cache_paths(cache_dir: str, split: str):
    base = os.path.join(cache_dir, split)
    return {
        "meta": base + "_meta.npz",
        "x": base + "_X.dat",
        "y": base + "_y.dat",
...
def build_or_load_split_memmaps(cfg_data: dict, split_name: str):
    """
    Build disk-backed memmaps for one split (train/val/test) if missing or rebuild=True.
    Returns: X_memmap, y_memmap
    """
    cache_dir = cfg_data.get("cache", {}).get("dir", "./cache_crnn")
    rebuild = bool(cfg_data.get("cache", {}).get("rebuild", False))
    ensure_dir(cache_dir)

    paths = _cache_paths(cache_dir, split_name)
    if (not rebuild) and os.path.exists(paths["meta"]) and os.path.exists(paths["x"]) and os.path.exists(paths["y"]):
        meta = np.load(paths["meta"], allow_pickle=True)
        X = np.memmap(paths["x"], mode="r", dtype=np.float32, shape=tuple(meta["x_shape"]))
        y = np.memmap(paths["y"], mode="r", dtype=np.float32, shape=tuple(meta["y_shape"]))
        return X, y

    splits = cfg_data.get("splits", {})
    if split_name not in splits:
        raise KeyError(f"cfg.data.splits.{split_name} missing")

    edf_dir = splits[split_name]["edf_dir"]
    json_dir = splits[split_name]["json_dir"]
    pairs = _pair_edf_json(edf_dir, json_dir)

    # pass 1: count total windows
    total_windows = 0
    x_shapes = []
    y_shapes = []
    for edf_path, json_path in pairs:
        X_tmp, y_tmp = load_edf_windows(edf_path, json_path, cfg_data)  # returns stacks per file
        total_windows += X_tmp.shape[0]
        x_shapes.append(X_tmp.shape)
        y_shapes.append(y_tmp.shape)

    if total_windows == 0:
        raise RuntimeError(f"No windows produced for split={split_name}")

    # allocate memmaps
    C = x_shapes[0][1]
    T = x_shapes[0][2]
    X_mm = np.memmap(paths["x"], mode="w+", dtype=np.float32, shape=(total_windows, C, T))
    y_mm = np.memmap(paths["y"], mode="w+", dtype=np.float32, shape=(total_windows, T))

    # pass 2: write sequentially
    w0 = 0
    for (edf_path, json_path) in pairs:
        X_tmp, y_tmp = load_edf_windows(edf_path, json_path, cfg_data)
        n = X_tmp.shape[0]
        X_mm[w0:w0+n] = X_tmp.astype(np.float32, copy=False)
        y_mm[w0:w0+n] = y_tmp.astype(np.float32, copy=False)
        w0 += n

    X_mm.flush(); y_mm.flush()
    np.savez(paths["meta"], x_shape=np.array(X_mm.shape), y_shape=np.array(y_mm.shape))
    # reopen read-only
...
def build_dataloaders(cfg):
    ...
    idx_tr = np.arange(X_tr.shape[0])
    idx_va = np.arange(X_va.shape[0])
    idx_te = np.arange(X_te.shape[0])

    ds_tr = EEGDataset(X_tr, y_tr, idx_tr,
                       normalize=sig.get("normalize", "zscore"),
                       reference=sig.get("reference", "car"),
                       channel_indices=channel_indices)
    ds_va = EEGDataset(X_va, y_va, idx_va,
                       normalize=sig.get("normalize", "zscore"),
                       reference=sig.get("reference", "car"),
                       channel_indices=channel_indices)
    ds_te = EEGDataset(X_te, y_te, idx_te,
                       normalize=sig.get("normalize", "zscore"),
                       reference=sig.get("reference", "car"),
                       channel_indices=channel_indices)
