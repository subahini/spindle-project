...
def evaluate_and_log(model, loader, device, threshold: float = 0.5, prefix: str = "val", step: int | None = None):
    """
    Run full evaluation, compute scalar metrics + plots, and log them to W&B.
    Assumes you defined: wandb.define_metric("<prefix>/*", step_metric="epoch")
    """
    y_true, y_probs, y_pred = collect_predictions(model, loader, device, threshold)

    # Always return metrics, even if W&B is off
    metrics = compute_metrics(y_true, y_probs, y_pred)

    if wandb is not None and wandb.run:
        # fall back to a sane epoch if step is None
        epoch_value = int(step) if step is not None else (wandb.run.step or 0)

        log = {"epoch": epoch_value}
        log.update({f"{prefix}/{k}": v for k, v in metrics.items()})

        # Only plot when both classes present
        if len(np.unique(y_true)) >= 2:
            cm_fig  = plot_confusion_matrix(y_true, y_pred, normalize=True)
            roc_fig = plot_roc_curve(y_true, y_probs, metrics["roc_auc"])
            pr_fig  = plot_pr_curve(y_true, y_probs, metrics["pr_auc"])
            log[f"{prefix}/confusion_matrix"] = wandb.Image(cm_fig)
            log[f"{prefix}/roc"] = wandb.Image(roc_fig)
            log[f"{prefix}/pr"] = wandb.Image(pr_fig)
            # close figs to free memory
            plt.close(cm_fig); plt.close(roc_fig); plt.close(pr_fig)

        # Important: don't pass step=... (we use the "epoch" field instead)
        wandb.log(log)
